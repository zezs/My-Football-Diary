...
"""in some situation this file might run multiple times, when we deploty this app, deplyment mechanism can run this app multiple times
  that would result in creating multiple different mongo clients one for each time the file runs,
   we solve this problem by using the app factory. 
   App factory enables deployment mechanism to not create "app" multiple times
"""
def create_app():
    load_dotenv()
    app = Flask(__name__)
